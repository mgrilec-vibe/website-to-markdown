...
import { compactSummarizableBlocks, createGroundingEvidence } from './extractive-summarizer';

const SUPPORTED_LANGUAGES: Readonly<Record<string, true>> = { en: true, es: true, ja: true, de: true, fr: true };
const CENTRALITY_COMPACTION_DETAILS = [65, 40, 20, 0] as const;
...
function normalizeUnit(unit: string | undefined): string {
  const normalized = unit?.toLocaleLowerCase().replace(/\s+/gu, '') ?? '';
  const aliases: Readonly<Record<string, string>> = {
    percent: '%', percentage: '%', milliseconds: 'ms', millisecond: 'ms', seconds: 's', second: 's',
    kilobytes: 'kb', kilobyte: 'kb', megabytes: 'mb', megabyte: 'mb', gigabytes: 'gb', gigabyte: 'gb',
  };
  return aliases[normalized] ?? normalized;
}

function factTokens(text: string): readonly string[] {
  const pattern = /(?<![\p{L}\p{N}])(v?\d+(?:[.,]\d+){0,3})(?:\s*(%|percent(?:age)?|ms|milliseconds?|s|seconds?|kb|kilobytes?|mb|megabytes?|gb|gigabytes?|x))?(?![\p{L}\p{N}])/giu;
  return [...text.matchAll(pattern)].map((match) => {
    const value = (match[1] ?? '').toLocaleLowerCase().replace(/,/gu, '');
    return `${value}${normalizeUnit(match[2])}`;
  }).filter(Boolean);
}
...
function hasGroundingSignal(text: string): boolean {
  ...
  return wordCount >= 8 && tokens.length >= 5;
}
...
export function validateSummaryGrounding(summaryMarkdown: string, sourceText: string): SummaryGroundingResult {
  ...
  if (!hasGroundingSignal(source)) return { supported: true };
  ...
  const sourceCode = new Set(inlineCode(sourceText));
  const unsupportedCode = inlineCode(summaryMarkdown).find((identifier) => !sourceCode.has(identifier));
  if (unsupportedCode) {
...
function blocksText(blocks: readonly MarkdownBlock[]): string {
  return blocks.map((block) => block.markdown).join('\n\n');
}

async function graphCompactedSource(
  session: SummarizerSession,
  blocks: readonly MarkdownBlock[],
  fullUsage: number,
  context?: string,
): Promise<{ readonly blocks: readonly MarkdownBlock[]; readonly fits: boolean } | undefined> {
  const shouldCompact = blocks.length >= 4 || (blocks.length >= 2 && fullUsage > session.inputQuota * 3);
  if (!shouldCompact) return undefined;
  let smallest: readonly MarkdownBlock[] | undefined;
  for (const detail of CENTRALITY_COMPACTION_DETAILS) {
    const compacted = compactSummarizableBlocks(blocks, detail);
    if (compacted.length === 0) continue;
    smallest = compacted;
    const usage = await measureInputUsage(session, blocksText(compacted), context);
    if (usage <= session.inputQuota) return { blocks: compacted, fits: true };
  }
  return smallest ? { blocks: smallest, fits: false } : undefined;
}
...
async function measuredChunks(
  ...
  const fullInput = blocksText(blocks);
  ...
  const compacted = await graphCompactedSource(session, blocks, fullUsage, context);
  if (compacted?.fits) return [compacted.blocks.slice() as MarkdownBlock[]];
  const sourceBlocks = compacted?.blocks ?? blocks;
  const chunks: MarkdownBlock[][] = [];
  let current: MarkdownBlock[] = [];
  for (const block of sourceBlocks) {
    const candidate = [...current, block];
    const input = blocksText(candidate);
...
function reductionInput(summaries: readonly GroundedSummary[], evidenceWordBudget: number): string {
  const plainDrafts = summaries.map((summary) => summary.markdown.trim()).filter(Boolean).join('\n\n');
  const sourceBlocks = uniqueSourceBlocks(summaries);
  const sourceText = blocksText(sourceBlocks);
  if (!hasGroundingSignal(sourceText)) return plainDrafts;
  const drafts = summaries
    .map((summary, index) => `### Draft ${index + 1}\n${summary.markdown.trim()}`)
    .join('\n\n');
  const evidence = evidenceWordBudget > 0
    ? createGroundingEvidence(sourceBlocks, evidenceWordBudget)
    : '';
  return [
    '## Draft summaries',
    drafts,
...
async function fitReductionInput(
  ...
  for (const evidenceWordBudget of evidenceBudgets) {
    const input = reductionInput(summaries, evidenceWordBudget);
...
export async function summarizeBlocks(
  ...
  for (const chunk of chunks) {
    ...
    const text = blocksText(chunk);
    summaries.push({ block: anchor, markdown: await groundedSummarize(session, text, text, context), sourceBlocks: chunk });
  }
