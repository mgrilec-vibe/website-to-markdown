...
The per-block Custom summary sentence count SHALL be a monotonic non-increasing function of `detail` for `detail ∈ [0, 100]` and SHALL always be at least one when `detail < 100`. The Custom export's measured **word count** for a fixed focused input SHALL be a monotonic non-increasing function of `detail` over the same range. The Custom export's measured **byte count** is best-effort: it is NOT required to be monotonic, because a block transitioning from retained to summarised adds the `> ` blockquote marker overhead and may produce a small byte increase at that transition.
