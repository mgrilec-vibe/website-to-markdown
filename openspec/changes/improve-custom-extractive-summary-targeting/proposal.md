...
- `src/export-compression.ts` — `detailPolicy` (curve + version bump), `withSummaries` (Custom branch only; bare `>` framing), the `ExportMetadata` initialiser `policyVersion: 2`. The `local-ai` branch and its `## Summary` heading are untouched. `relevanceScores` and `extractiveSummaries` targeting are NOT changed in this increment.
